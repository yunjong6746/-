...
export default function MobileLogAnalyzer() {
      ...
      const response = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [
            {
              role: "user",
              content: `다음 모바일 성능 로그를 분석하고 리포트를 생성해주세요:\n\n\`\`\`\n${logText}\n\`\`\``,
            },
          ],
        }),
      });
  ...
  const formatReport = (text) => {
    ...
    return lines.map((line, i) => {
        ...
        return (
          <div
            ...
            style={{
              ...
              fontWeight: 700,
              color: isH2 ? "#00e5ff" : "#b0bec5",
              marginTop: isH2 ? "20px" : "12px",
              marginBottom: "6px",
              letterSpacing: "0.05em",
              textTransform: isH2 ? "uppercase" : "none",
              borderBottom: isH2 ? "1px solid #1a3a4a" : "none",
              paddingBottom: isH2 ? "4px" : "0",
            }}
          >
            {content}
          </div>
      ...
      if (line.includes("🚨") || line.toLowerCase().includes("critical") || line.toLowerCase().includes("fatal")) {
        return (
          <div key={i} style={{ color: SEVERITY_COLOR.critical, fontSize: "12px", marginBottom: "3px", fontFamily: "monospace" }}>
            {line}
          </div>
        );
      }
      if (line.includes("⚠️") || line.toLowerCase().includes("warning")) {
        return (
          <div key={i} style={{ color: SEVERITY_COLOR.warning, fontSize: "12px", marginBottom: "3px", fontFamily: "monospace" }}>
            {line}
          </div>
        );
      }
      if (line.startsWith("- ") || line.match(/^\d+\./)) {
        return (
          <div key={i} style={{ color: "#cfd8dc", fontSize: "12px", marginBottom: "3px", paddingLeft: "12px", fontFamily: "monospace" }}>
            {line}
          </div>
        );
      }
      if (line.startsWith("**") || line.includes(": ")) {
        return (
          <div key={i} style={{ color: "#90a4ae", fontSize: "12px", marginBottom: "3px", fontFamily: "monospace" }}>
            {line}
          </div>
        );
      }
      return (
        <div key={i} style={{ color: "#78909c", fontSize: "12px", marginBottom: "2px", fontFamily: "monospace" }}>
          {line}
        </div>
      );
    });
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          background: "linear-gradient(90deg, #001929 0%, #002a3d 50%, #001929 100%)",
      ...
      >
        ...
        <div>
          <div style={{ fontSize: "15px", fontWeight: 700, color: "#00e5ff", letterSpacing: "0.1em" }}>
            MOBILE LOG ANALYZER
          </div>
          <div style={{ fontSize: "9px", color: "#546e7a", letterSpacing: "0.2em" }}>
            POWERED BY CLAUDE AI · 성능 이슈 자동 탐지
          </div>
        </div>
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              color: activeTab === tab.key ? "#00e5ff" : "#37474f",
              cursor: "pointer",
              transition: "all 0.2s",
            }}
          >
        ...
        {activeTab === "input" && (
          <>
            ...
            <div style={{ display: "flex", gap: "8px", alignItems: "center" }}>
              ...
              <button onClick={() => loadExample("android")} style={exBtnStyle}>
                ANDROID CRASH
              </button>
              <button onClick={() => loadExample("cpu")} style={exBtnStyle}>
                CPU/MEMORY
              </button>
            </div>
            ...
            <button
              ...
              style={{
                padding: "14px",
                background: loading ? "#001929" : "linear-gradient(90deg, #004d66, #006680)",
                border: "1px solid #00e5ff44",
                color: loading ? "#37474f" : "#00e5ff",
                fontFamily: "inherit",
                fontSize: "11px",
                fontWeight: 700,
                letterSpacing: "0.2em",
      ...
      <style>{`
        ...
        textarea::-webkit-scrollbar-track { background: #060d14; }
        textarea::-webkit-scrollbar-thumb { background: #0d2233; }
        div::-webkit-scrollbar { width: 4px; }
        div::-webkit-scrollbar-track { background: #060d14; }
        div::-webkit-scrollbar-thumb { background: #0d2233; }
      `}</style>
    </div>
  );
}
